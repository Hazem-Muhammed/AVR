...
dio_level_t dio_dioLevelReadChannel(dio_portId_t port, dio_channelId_t channel)
{
	...
	{
	 case DIO_PORTA:
		loc_pinRead = GET_BIT(DIO_PINA_REG, channel);
		break;

	 case DIO_PORTB:
		loc_pinRead = GET_BIT(DIO_PINB_REG, channel);
		break;

	 case DIO_PORTC:
		loc_pinRead = GET_BIT(DIO_PINC_REG, channel);
		break;

	 case DIO_PORTD:
		loc_pinRead = GET_BIT(DIO_PIND_REG, channel);
		break;
    }
	return loc_pinRead;
}
...
void dio_vidEnablePullUp(dio_portId_t port, dio_channelId_t channel)
{
	...
	{
		case DIO_PORTA :
		if(GET_BIT(DIO_DDRA_REG,channel) == 0)
		SET_BIT(DIO_PORTA_REG,channel);
		break;

		case DIO_PORTB :
		if(GET_BIT(DIO_DDRB_REG,channel) == 0)
		SET_BIT(DIO_PORTB_REG,channel);
		break;

		case DIO_PORTC :
		if(GET_BIT(DIO_DDRC_REG,channel) == 0)
		SET_BIT(DIO_PORTC_REG,channel);
		break;

		case DIO_PORTD :
		if(GET_BIT(DIO_DDRD_REG,channel) == 0)
		SET_BIT(DIO_PORTD_REG,channel);
		break;
	}
